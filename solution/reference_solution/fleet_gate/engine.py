"""Gate simulation engine (policy-correct)."""
...
from fleet_gate.ledger import SlotState, TeamLedger, can_borrow, reclaim_amount
from fleet_gate.scheduler import WaitItem, wait_sort_key
from fleet_gate.window import slot_active, slot_expired
...
class GateEngine:
    ...
    def _reserve_slot(self, ev: dict[str, Any]) -> dict[str, Any]:
        ...
        if slot_active(st.start_ms, st.end_ms, int(ev["ts"])):
            led.available += grant
        ...
        for led in self.ledgers.values():
            for st in list(led.slots.values()):
                ...
                if not slot_expired(st.start_ms, st.end_ms, t):
                    continue
                amt = reclaim_amount(st.grant, st.consumed)
    ...
    def _drain_queue(self, t: int) -> list[dict[str, Any]]:
        del t
        rows: list[dict[str, Any]] = []
        self.queue.sort(key=wait_sort_key)
        remaining: list[WaitItem] = []
        for item in self.queue:
            led = self._ledger(item.team)
            if led.available >= item.cost:
    ...
    def _spend(self, ev: dict[str, Any]) -> dict[str, Any]:
        ...
        if can_borrow(led.tier, cap, led.borrowed, need):
            if led.available:
                if slot_id in led.slots:
    ...
    def simulate(self, events: list[dict[str, Any]], at_ms: int | None = None) -> list[dict[str, Any]]:
        ...
        end_t = at_ms if at_ms is not None else (events[-1]["ts"] if events else 0)
